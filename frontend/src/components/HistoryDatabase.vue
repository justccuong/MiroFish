<template>
  ...
  >
    ...
    <div class="section-header">
      ...
      <span class="section-title">{{ $t('history.sim_history') }}</span>
      <div class="section-line"></div>
    </div>
        ...
        <div class="card-header">
          ...
          <div class="card-status-icons">
            <span 
              ...
              :title="$t('step1.graph_build.title')"
            >◇</span>
            <span 
              class="status-icon available" 
              :title="$t('step2.init_sim.title')"
            >◈</span>
            <span 
              class="status-icon" 
              :class="{ available: project.report_id, unavailable: !project.report_id }"
              :title="$t('step4.header.prediction_report')"
            >◆</span>
          </div>
        </div>
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-list" v-if="project.files && project.files.length > 0">
            ...
            <div v-if="project.files.length > 3" class="files-more">
              +{{ project.files.length - 3 }} {{ $t('common.items') }}
            </div>
          </div>
          <!-- 无文件时的占位 -->
          <div class="files-empty" v-else>
            <span class="empty-file-icon">◇</span>
            <span class="empty-file-text">{{ $t('history.no_history') }}</span>
          </div>
        </div>
    ...
    <div v-if="loading" class="loading-state">
      ...
      <span class="loading-text">{{ $t('graph.loading') }}</span>
    </div>
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="modal-section">
                <div class="modal-label">{{ $t('history.sim_requirement') }}</div>
                <div class="modal-requirement">{{ selectedProject.simulation_requirement || $t('common.no') }}</div>
              </div>

              <!-- 文件列表 -->
              <div class="modal-section">
                <div class="modal-label">{{ $t('history.related_files') }}</div>
                ...
                <div class="modal-empty" v-else>{{ $t('history.no_history') }}</div>
              </div>
            </div>
            ...
            <div class="modal-actions">
              ...
              >
                ...
                <span class="btn-text">{{ $t('step1.graph_build.title') }}</span>
              </button>
              <button 
              ...
              >
                ...
                <span class="btn-icon">◈</span>
                <span class="btn-text">{{ $t('step2.init_sim.title') }}</span>
              </button>
              <button 
              ...
              >
                ...
                <span class="btn-icon">◆</span>
                <span class="btn-text">{{ $t('step4.header.prediction_report') }}</span>
              </button>
            </div>
            <!-- 不可回放提示 -->
...
const getSimulationTitle = (requirement) => {
  if (!requirement) return t('history.unnamed_sim')
...
const formatRounds = (simulation) => {
  const current = simulation.current_round || 0
  const total = simulation.total_rounds || 0
  if (total === 0) return t('common.pending')
...
const truncateFilename = (filename, maxLength) => {
  if (!filename) return t('history.unknown_file')
...
const loadHistory = async () => {
  try {
    ...
    console.error(`${t('common.unknown_error')}:`, error)
    projects.value = []
  } finally {
    loading.value = false
  }
